...
def buttonEvent(button, held) {
	...
	if (held) {
		result << createEvent(name: "held", value: button, descriptionText: "$device.displayName button $button was held", isStateChange: true);
	} else {
		result << createEvent(name: "pushed", value: button, descriptionText: "$device.displayName button $button was pushed", isStateChange: true);
	}

	def buttons = (device.currentState("numberOfButtons").value).toBigInteger();
...
def ping() {
	log.trace "Ping is not implemented";
}
